...
(function () {
	...
	const initOurStory = async function () {
		try {
			...
			const ourStoryButtonRef = document.querySelector('[data-our-story-button]');
			...
			gsap.to(bigCircleRef, {
				...
				scrollTrigger: {
					...
					scrub: 3, // Slower scrubbing with 2s lag for slower rotation
				},
			});
			...
			gsap.to(btnInsideRef, {
				...
				scrollTrigger: {
					...
					onUpdate: function(self) {
						// Add class to data-our-story-button when progress reaches 80%
						if (ourStoryButtonRef) {
							if (self.progress >= 0.8) {
								ourStoryButtonRef.classList.add('btn_animation_complete');
							} else if (self.progress < 0.8) {
								// Remove class if scrolling back up before 80%
								ourStoryButtonRef.classList.remove('btn_animation_complete');
							}
						}
					},
				},
	...
	const initModalSwiper = function () {
		...
		if (swiperElement._swiperInstance || swiperElement.swiper) {
			console.log('OurStory Modal: Swiper already initialized, skipping');
			return;
		}
		...
		// Get navigation elements - buttons are in a sibling container, not inside swiper
		// Find the modal container first
		const modalContainer = swiperElement.closest('.popup-modal-content') || swiperElement.closest('.popup-modal-container');
		const nextButton = modalContainer ? modalContainer.querySelector('.pop-up-section-arrows .popup_slider_next_btn') : document.querySelector('.pop-up-section-arrows .popup_slider_next_btn');
		const prevButton = modalContainer ? modalContainer.querySelector('.pop-up-section-arrows .popup_slider_prev_btn') : document.querySelector('.pop-up-section-arrows .popup_slider_prev_btn');

		// Debug: Log if buttons are found
		if (!nextButton || !prevButton) {
			console.warn('OurStory Modal: Navigation buttons not found', {
		...
		try {
			...
			let activeNextButton = nextButton;
			let activePrevButton = prevButton;

			// Update button states on slide change
			// Swiper's navigation config handles the clicks, we just manage the disabled states
			const updateButtonStates = function () {
				if (activeNextButton) {
					if (modalSwiper.isEnd) {
						activeNextButton.classList.add('swiper-button-disabled');
						activeNextButton.setAttribute('disabled', 'disabled');
					} else {
						activeNextButton.classList.remove('swiper-button-disabled');
						activeNextButton.removeAttribute('disabled');
					}
				}
				if (activePrevButton) {
					if (modalSwiper.isBeginning) {
						activePrevButton.classList.add('swiper-button-disabled');
						activePrevButton.setAttribute('disabled', 'disabled');
					} else {
						activePrevButton.classList.remove('swiper-button-disabled');
						activePrevButton.removeAttribute('disabled');
					}
				}
			};
