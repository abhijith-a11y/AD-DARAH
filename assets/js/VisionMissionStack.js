/**
 * Vision Mission Stack Component JavaScript
...
(function () {
	...
	/**
	 * Initialize Vision Mission Stack Component
	 */
	const initVisionMissionStack = function () {
		const cards = document.querySelectorAll("[data-vision-mission-card]");
		...
		if (typeof gsap === "undefined" || typeof ScrollTrigger === "undefined") {
			console.warn("VisionMissionStack: GSAP or ScrollTrigger not loaded");
			return;
		}
	...
	const waitForReady = function () {
		if (document.readyState === "loading") {
			document.addEventListener("DOMContentLoaded", function () {
				setTimeout(initVisionMissionStack, 100);
			});
		} else {
			setTimeout(initVisionMissionStack, 100);
		}
	...
	const retryInit = function () {
		if (retryCount < maxRetries) {
			...
			setTimeout(function () {
				if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
					initVisionMissionStack();
				} else {
					retryInit();
				}
			}, 200);
		} else {
			console.error("VisionMissionStack: Failed to load GSAP after multiple retries.");
		}
	};
